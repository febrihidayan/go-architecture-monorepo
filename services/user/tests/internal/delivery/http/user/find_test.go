...
func (x *UserHandlerSuite) TestFind() {
	testCases := []struct {
		name  string
		param HandlerParams
	...
	}{
		{
			...
			mock: func(m HandlerParams) {
				x.UserUsecase.On("Find", mock.Anything, mock.Anything).Return(&entities.User{}, nil)
			},
		},
		{
			...
			mock: func(m HandlerParams) {
				x.UserUsecase.On("Find", mock.Anything, mock.Anything).Return(nil, x.Error)
			},
		},
	}
