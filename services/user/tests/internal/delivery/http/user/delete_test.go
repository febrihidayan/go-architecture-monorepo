...
func (x *UserHandlerSuite) TestDelete() {
	testCases := []struct {
		name  string
		param HandlerParams
	...
	}{
		{
			...
			mock: func(m HandlerParams) {
				x.UserUsecase.On("Delete", mock.Anything, mock.Anything).Return(nil)
			},
		},
		{
			...
			mock: func(m HandlerParams) {
				x.UserUsecase.On("Delete", mock.Anything, mock.Anything).Return(x.Error)
			},
		},
	}
	...
	for _, tc := range testCases {
		x.Run(tc.name, func() {
			...
			tc.mock(tc.param)
			http.HandlerFunc(x.Http.Delete).ServeHTTP(x.Response, re)
