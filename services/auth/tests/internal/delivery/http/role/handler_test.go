...
import (
	...
	role_http "github.com/febrihidayan/go-architecture-monorepo/services/auth/internal/delivery/http/delivery/role"
...
type RoleHandlerSuite struct {
	suite.Suite
	Route       *mux.Router
	Cfg         *config.AuthConfig
	Http        *role_http.RoleHttpHandler
	Response    *httptest.ResponseRecorder
	RoleUsecase *usecases.RoleUsecaseMock
	Error       *exceptions.CustomError
	Token       string
	Id          common.ID
}

func (x *RoleHandlerSuite) SetupTest() {
	x.RoleUsecase = new(usecases.RoleUsecaseMock)

	x.Response = httptest.NewRecorder()
	x.Route = mux.NewRouter()

	x.Http = &role_http.RoleHttpHandler{
		Cfg:         x.Cfg,
		RoleUsecase: x.RoleUsecase,
	}
...
func TestAclHandler(t *testing.T) {
	suite.Run(t, new(RoleHandlerSuite))
}
