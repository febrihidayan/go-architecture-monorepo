...
func (x *PermissionHandlerSuite) TestGetAll() {
	testCases := []struct {
		name  string
		param HandlerParams
	...
	}{
		{
			...
			mock: func(m HandlerParams) {
				x.PermissionUsecase.On("GetAll", mock.Anything, mock.Anything).Return(&entities.PermissionMeta{}, nil)
			},
		},
	}
	...
	for _, tc := range testCases {
		x.Run(tc.name, func() {
			x.SetupTest()
			var params request.PermissionQueryParams
