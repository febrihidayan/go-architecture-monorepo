...
func (x *AclHandlerSuite) TestUpdatePermissionByRole() {
	...
	testCases := []struct {
	...
	}{
		{
			...
			mock: func(m HandlerParams) {
				x.AclUsecase.On("UpdatePermissionByRole", mock.Anything, mock.Anything).Return(nil)
			},
		},
		{
			...
			mock: func(m HandlerParams) {
				x.AclUsecase.On("UpdatePermissionByRole", mock.Anything, mock.Anything).Return(x.Error)
			},
		},
	}
	...
	for _, tc := range testCases {
		x.Run(tc.name, func() {
			...
			tc.mock(tc.param)
			http.HandlerFunc(x.Http.UpdatePermissionByRole).ServeHTTP(x.Response, re)
