...
func (x *AuthHandlerSuite) TestPasswordReset() {
	...
	testCases := []struct {
	...
	}{
		{
			...
			mock: func(m HandlerParams) {
				x.AuthUsecase.On("PasswordReset", mock.Anything, mock.Anything).Return(&entities.AclMeta{}, nil)
			},
		},
		{
			...
			mock: func(m HandlerParams) {
				x.AuthUsecase.On("PasswordReset", mock.Anything, mock.Anything).Return(nil, x.Error)
			},
		},
	}
	...
	for _, tc := range testCases {
		x.Run(tc.name, func() {
			...
			body := tc.param.payload.Get(1).(request.AuthPasswordResetRequest)

			req := httptest.NewRequest(tc.param.method, tc.param.path, BodyHelper(body))
			req.Header.Set("Authorization", token)

			tc.mock(tc.param)
			http.HandlerFunc(x.Http.PasswordReset).ServeHTTP(x.Response, req)
