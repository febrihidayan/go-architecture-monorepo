...
func (x *AuthHandlerSuite) TestPasswordEmail() {
	...
	testCases := []struct {
	...
	}{
		{
			...
			mock: func(m HandlerParams) {
				x.AuthUsecase.On("PasswordEmail", mock.Anything, mock.Anything).Return(&entities.AclMeta{}, nil)
			},
		},
		{
			...
			mock: func(m HandlerParams) {
				x.AuthUsecase.On("PasswordEmail", mock.Anything, mock.Anything).Return(nil, x.Error)
			},
		},
	}
	...
	for _, tc := range testCases {
		x.Run(tc.name, func() {
			...
			tc.mock(tc.param)
			http.HandlerFunc(x.Http.PasswordEmail).ServeHTTP(x.Response, req)
