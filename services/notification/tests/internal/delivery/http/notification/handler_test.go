...
import (
	...
	notification_http "github.com/febrihidayan/go-architecture-monorepo/services/notification/internal/delivery/http/delivery/notification"
...
type NotificationHandlerSuite struct {
	...
	Http                *notification_http.NotificationHttpHandler
	Response            *httptest.ResponseRecorder
	NotificationUsecase *usecases.NotificationUsecaseMock
	Error               *exceptions.CustomError
	Token               string
	Id                  common.ID
}

func (x *NotificationHandlerSuite) SetupTest() {
	x.NotificationUsecase = new(usecases.NotificationUsecaseMock)

	x.Response = httptest.NewRecorder()
	x.Route = mux.NewRouter()

	x.Http = &notification_http.NotificationHttpHandler{
		Cfg:                 x.Cfg,
		NotificationUsecase: x.NotificationUsecase,
	}
