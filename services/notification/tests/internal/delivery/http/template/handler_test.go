...
import (
	...
	template_http "github.com/febrihidayan/go-architecture-monorepo/services/notification/internal/delivery/http/delivery/template"
...
type TemplateHandlerSuite struct {
	...
	Http            *template_http.TemplateHttpHandler
	Response        *httptest.ResponseRecorder
	TemplateUsecase *usecases.TemplateUsecaseMock
	Error           *exceptions.CustomError
	Token           string
	Id              common.ID
}

func (x *TemplateHandlerSuite) SetupTest() {
	x.TemplateUsecase = new(usecases.TemplateUsecaseMock)

	x.Response = httptest.NewRecorder()
	x.Route = mux.NewRouter()

	x.Http = &template_http.TemplateHttpHandler{
		Cfg:             x.Cfg,
		TemplateUsecase: x.TemplateUsecase,
	}
